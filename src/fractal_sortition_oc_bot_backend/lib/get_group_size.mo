...
module {
  ...
  public func getGroupSize(number_of_participants : Nat, optimization_mode : Types.OptimizationMode) : Nat {
    ...
    while (group_size > 3) {
      ...
      if ((number_of_participants / group_size) >= mininum_number_of_groups) {
        return group_size;
      };
